...
class _SecondApp extends State<SecondApp> {
  final nameController = TextEditingController(); //텍스트필드에 변화를 감지하는 함수
  int? _radioValue=0; //라디오 버튼 인덱스 초기 값
  bool? flyExist = false;
  String? _imagePath;
  @override
    ...
    return Scaffold(
      body: Container(
        child: Column(
            ...
            Row(
                children: <Widget> [ // 라디오 버튼 위젯
                Radio(value: 0,groupValue: _radioValue, onChanged: _radioChange), //radio 버튼 생성 value 버튼 인덱스 값, 그룹화, 이벤트 처리
                Text('양서류'),
            ...
            Container(
              height: 100,
              child: ListView( // 동물 아이템 목록 리스트 뷰로 출력
            ...
            ElevatedButton( // 상자 버튼
                child: Text('동물 추가하기'),
                onPressed:() { // 이벤트 처리
                  var animal = Animal( //객체 animal 선언
                      animalName: nameController.value.text,
                      kind: getKind(_radioValue),
                      imagePath: _imagePath,
                      flyExist: flyExist);
                  AlertDialog dialog = AlertDialog( //팝업 기능
                    title: Text('동물 추가하기'),
                    content: Text(
                    ...
                    actions: [
                      ElevatedButton( //'예'를 클릭시 동물 아이템 리스트에 추가 버튼
                        onPressed: () {
